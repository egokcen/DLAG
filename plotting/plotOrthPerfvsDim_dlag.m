...
for groupIdx = 1:numGroups
    ...
    legendEntries(end+1) = errorbar(1:length(res.R2orth_denoise.indiv{groupIdx}), ...
    ...
    if ismember(groupIdx,rGroups)
        ...
        legendEntries(end+1) = errorbar(1:length(res.R2orth_reg.indiv(:,predIdx)), ...
    ...
    % Based on denoised prediction
    % Including all types of latents
    legendEntries(end+1) = errorbar(1:length(res.MSEorth_denoise.indiv{groupIdx}), ...
    ...
    if ismember(groupIdx,rGroups)
        ...
        predGroup = setdiff(rGroups, groupIdx);
        predIdx = find(rGroups == predGroup);
        legendEntries(end+1) = errorbar(1:length(res.MSEorth_reg.indiv(:,predIdx)), ...
