function k = transformed_gpcov_mixdemo(V, params, binWidth, varargin)
%
% k = transformed_gpcov_mixdemo(V, params, binWidth, ...)
%
% Description: Given a coupled set of basis vectors, V, compute GP
%              covariance functions for projections that lie along those
%              directions. Plot the mixing distributions with the
%              transformed covariance function.
%
% Arguments:
%
%     Required:
...
%
% Revision history:
%     04 Mar 2023 -- Initial full revision.
...
mix = cell(1,r);
for j = 1:r
    mix{j} = cell(numGroups,numGroups);
    for groupIdx1 = 1:numGroups
        for groupIdx2 = 1:numGroups
            ...
            for xIdx = 1:xDim_across
                ...
                mix{j}{groupIdx1,groupIdx2}{end+1} = ...
                    @(t) ka_prior{groupIdx1,groupIdx2}{xIdx}(t) ...
                    .*(V{groupIdx1}(:,j)'*Ca{groupIdx1}(:,xIdx)*Ca{groupIdx2}(:,xIdx)'*V{groupIdx2}(:,j));
            end
    ...
    for j = 1:r
        for groupIdx1 = 1:numGroups
            for groupIdx2 = 1:numGroups
                ...
                for xIdx = 1:length(mix{j}{groupIdx1,groupIdx2})
                    mix{j}{groupIdx1,groupIdx2}{xIdx} ...
                        = @(t)  k{j}{groupIdx1,groupIdx1}(0).^(-0.5) ...
                         .* mix{j}{groupIdx1,groupIdx2}{xIdx}(t) ...
                         .* k{j}{groupIdx2,groupIdx2}(0).^(-0.5);
                end
            end
        end
    ...
    for j = 1:r
        ...
        for groupIdx1 = 1:numGroups
            for groupIdx2 = 1:numGroups
                ...
                plot(lagSteps, k_curr, 'k-', 'linewidth', 3);
                set(gca,'ColorOrderIndex',1)
                for xIdx = 1:length(mix{j}{groupIdx1,groupIdx2})
                    mix_curr = mix{j}{groupIdx1,groupIdx2}{xIdx}(lagSteps);
                    [maxval, maxdelay] = max(abs(mix_curr));
                    if mix_curr(maxdelay) < 0
                        mix_curr = -1.*mix_curr; 
                    end
                    plot(lagSteps, mix_curr, '-', 'linewidth', 1.5);
                end
                axis square;
                xlabel('Time lag (ms)');
                ylabel(ylbl);
